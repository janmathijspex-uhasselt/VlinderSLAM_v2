...
// Produce 32-bit microsecond timestamp for a sensor event
static uint32_t touSTimestamp32(uint32_t hostInt, int referenceDelta, uint16_t delay)
{
    uint32_t timestamp = hostInt + (referenceDelta + delay) * 100;
    return timestamp;
}
    ...
    while (cursor < len) {
        ...
        if (reportLen == 0) {
            ...
            if (reportId == SENSORHUB_BASE_TIMESTAMP_REF) {
                ...
                // event.timestamp_uS = touSTimestamp(timestamp, referenceDelta, delay); // Weg gecomment van originele lib!
                event.timestamp32_uS = touSTimestamp32(timestamp, referenceDelta, delay);

                event.timebase = referenceDelta;
                event.delay = delay;
